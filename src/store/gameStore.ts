...
export const useGameStore = create<GameStoreState>((set, get) => ({
    ...
    set((state) => {
      ...
      if (state.gameState.playerSchoolId && currentWeek === 8 && nextWeek >= 9) { // >=9 handles if we skip over 9, but normally we land on 9
          ...
          if (pSchool) {
             ...
             // Samba selection is generated by lockInEnredo(), not here.
          }
      }
      ...
      if (state.gameState.playerSchoolId) {
        ...
        if (pSchoolIdx !== -1) {
            ...
            // 1. Research Progress (Weekly — fractional, staff-aware)
            if (pSchool.researchFocusId && pSchool.enredoCandidates) {
                ...
                if (candIdx !== -1) {
                    ...
                    const increment = calculateWeeklyResearchIncrement(pSchool);
                    ...
                    // Reveal one stat for each whole number crossed
  ...
  simulateMarketAndJump: () =>
    set((state) => {
      ...
      // Step 1: Run AI transfers for 8 weeks (simulate the full market for AI schools)
      ...
      // Step 2: Auto-staff the player's school greedily
      ...
      for (const role of unfilledRoles) {
        // Find best available staff for this role, sorted by reputation descending
      ...
      // Step 3: Auto-pick the best enredo from the pool by potentialScore
      ...
      // Step 4: Generate samba selection (same as lockInEnredo does)
      let pendingSambaSelection = state.gameState.pendingSambaSelection;
      if (playerSchool.enredo && !state.gameState.chosenSambaEnredo) {
        pendingSambaSelection = generateSambaSelectionProcess(playerSchool.enredo, playerSchool);
      }
